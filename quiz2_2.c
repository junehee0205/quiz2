...
int main(int argc, char *argv[])
{
	...
	pthread_t consumer;
	pthread_create(&producer, NULL, consumer, &buffer);
...
void Buffer_Init(Buffer *buf, int size)
{
	...
	sem_init(&buf->producer, 0, 1);	
	sem_init(&buf->consumer, 0, 1);	
	buf->in = buf->out = 0;
	buf->size = size;
}
...
void Buffer_Destroy(Buffer *buf)
{
	...
	sem_destroy(&buf->producer);
	sem_destroy(&buf->consumer);
...
int Buffer_Delete(Buffer *buf)
{
	...
	for(int i = buf->in; i<0; i--){
		ret = buf->buffer[buf->in];
		buf->in = (buf->in - 1) % buf->size;
	}
